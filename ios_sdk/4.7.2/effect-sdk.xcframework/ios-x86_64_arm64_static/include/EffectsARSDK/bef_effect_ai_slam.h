...
typedef struct bef_ai_slam_camera_extrinsic{
    float tic[3];  // 3x1 translation matrix
    float qic[4];  // quaternion : xyzw
    float td;
}bef_ai_slam_camera_extrinsic;

typedef struct bef_ai_slam_camera_distort{
    float k1;
    float k2;
    float p1;
    float p2;
}bef_ai_slam_camera_distort;
...
typedef struct bef_ai_slam_camera_info
{
    ...
    bef_ai_slam_camera_extrinsic camera_extrinsic;
    bef_ai_slam_camera_distort camera_distort;
    ...
    int run_gba;                            // use gba
    int easy_init;                          // use fast initialization
    int width;
    int height;
    int enable_fusion;
    ...
    bool self_calib;
    int enable_depth;
    bool is_depth_inverse;
    int robust_depth_align;
...
typedef struct bef_ai_slam_pose{
    ...
    float img_td;        // t_imu = t_img + img_td, unit: ms
    int is_stationary;   // -1 for system uninitialized, 0 for static, 1 for dynamic, 2 for not sure
