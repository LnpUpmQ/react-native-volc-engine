...
#define AI_3D_SKELETON_SMPL_HEATMAP_KEYPOINT_NUM 29
...
typedef struct bef_ai_skeleton3d_target {
  ...
  float smpl_heatmap_kpts_2d[AI_3D_SKELETON_SMPL_HEATMAP_KEYPOINT_NUM*2]; // heatmap 关键点的数量
  float box[4]; // 输入网络的人体框
  int joint_num; // 3D关键点个数
  int tracking_id; // 目标id
  int new_target; // 是否是新目标
  int is_smpl_heatmap; //是否是smpl格式的heatmap
